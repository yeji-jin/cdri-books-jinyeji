...
- **접근성**: `header`/`nav`/`main` 등 시멘틱 태그로 레이아웃을 구성하고, 페이지마다 의미있는 `h1`을 하나씩만 두었습니다. 검색 인풋·상세검색 폼 필드엔 `aria-label`로 접근 가능한 이름을 부여했습니다. 상세검색 토글 버튼과 커스텀 `Select`는 `aria-expanded`/`aria-haspopup`/`role="listbox"`·`role="option"`으로 열림 상태와 위젯 종류를 스크린리더에 전달하고, 폼 에러 메시지는 `aria-describedby`로 해당 인풋과 연결했습니다. 장식용 아이콘은 `aria-hidden`으로 감춰 중복 안내를 없앴습니다.
